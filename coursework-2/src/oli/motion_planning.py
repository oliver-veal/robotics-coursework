from math import cos, sin, acos, asin, atan2, sqrt, pi, ceil
...
class MotionPlanner():
    ...
    def send_velocity(self, vref):
        ...
        print("vx: {:.2f}, vy: {:.2f}, v_fwd: {:.2f}, w: {:.2f}".format(vref[0], vref[1], v_forward, omega))
    ...
    def setup_waypoints(self):
        ...
        print(self.world_position(np.array([208, 128])))
    ...
    def potential_field(self, deniro_pos=None):
        if deniro_pos is not None:
            deniro_position = deniro_pos
        ############################################################### TASK C
        complete = False
        coords = self.map_position(deniro_position)
        if self.pixel_map[int(coords[0, 1]), int(coords[0, 0])] == 1:
        ...
        max_force, mfv = 0, []
        mags = np.linalg.norm(obstacle_force, axis=1)
        avg = 40
        ind = np.argpartition(mags, -avg)[-avg:]
        #for i in range(obstacle_force.shape[0]):
         #   if mags[i] > max_force:
          #      max_force, mfv = mags[i], obstacle_force[i]
        negative_force = K_rep * np.sum(obstacle_force[ind], axis=0) / avg#, axis=0)# / obstacle_pixel_locations.shape[0]
    ...
    def generate_random_points(self, N_points):
        ...
        #    pixel_points = np.random.randint(0, 320, (N_points - N_accepted, 2))  # generate random coordinates
        ...
        N_points = 100 # Number of nodes in graph
        step = 2
        ...
        accepted_points = np.array(accepted_points)#accepted_points[1:, :]
        rejected_points = np.array([])#rejected_points[1:, :]   
    ...
    if task == 'waypoints':
        ...
        for y in range(0, velmap.shape[0], step):
            for x in range(0, velmap.shape[1], step):
                ...
                force, complete = planner.potential_field(deniro_pos = planner.world_position(np.array([x, y])))
        ...
        plt.imshow(planner.pixel_map, vmin=0, vmax=1, origin='lower', cmap="Greys")
        plt.show()
        #plt.imshow(velmap, origin="lower")
        #plt.show()
        #plt.imshow(velmapx, origin="lower")
        #plt.show()
        #plt.imshow(velmapy, origin="lower")
        #plt.show()

if __name__ == "__main__":
    tasks = ['waypoints', 'potential', 'prm', 'heatmap']
    if len(sys.argv) <= 1:
